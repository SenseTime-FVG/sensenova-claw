...
### 2026-04-15 外部 CLI 仓库可跑性验证补充

成功经验：
- 在当前受限环境排查第三方 CLI 仓库时，先区分“仓库本身失败”和“环境只读导致的假失败”很关键；这次 `xiaohongshu-cli` 的 pytest 首轮报错都是因为默认写 `~/.xiaohongshu-cli`，把 `HOME` 切到 `/tmp` 后同一批测试即可通过。
- 对带真实联调测试的仓库，优先跑其官方 CI 等价命令（本次是 `pytest --ignore=tests/test_integration.py`）最容易判断“基础能力是否可用”，再单独分析登录态、浏览器、网络这些外部前置条件。

失败/风险经验：
- 没有浏览器安装、没有现成登录态时，这类“依赖浏览器 cookie / 二维码登录”的抓取型 CLI 即使单测全绿，也不能误判为“本机已可直接抓取真实数据”；还需要额外满足浏览器、cookie 或扫码登录链路。

### 2026-04-15 Deep Research 竞品调研补充

成功经验：
- 做 AI 产品竞品调研时，先按“报告型研究代理 / 企业研究代理 / 开源底座 / 相邻超代理”分层，再收资料，能避免把 Manus、Genspark 这类广义 agent 产品和 OpenAI、Perplexity 这类 deep research 产品混在一起。
- 对时效性很强的产品能力，优先组合使用“官方产品页 + 官方博客/发布时间线 + 官方帮助中心 + 定价页 + 官方 GitHub”；功能、配额、价格、地区可用性分别从最对应的页面取数，冲突最少。
- 开源竞品分析里，记录 `license`、GitHub stars、是否支持 MCP/私域数据/本地部署，通常比罗列功能点更有决策价值，更能判断项目适不适合作为产品底座。

失败/风险经验：
- 只看媒体稿或二手测评，很容易把“首发 beta 能力”和“当前线上能力”混为一谈；写 deep research 竞品文档时必须标具体日期。
- 一些官网会按地区、语言或动态渲染隐藏价格字段；如果抓取拿不到明确数值，应写成“按计划分级提供”或“需销售咨询”，不要硬填未经当前官方页面验证的旧价格。

### 2026-04-15 Deep Research Benchmark 调研与飞书落文档补充

成功经验：
- 对“调研外部 benchmark 并输出结论”这类任务，先按“直接评估端到端 deep research / 评估关键子能力 / 容易被误用的补充 benchmark”三层分类，比按论文发布时间平铺更利于最终选型。
- 这类 benchmark 调研优先引用官方项目页、论文页、数据集卡和官方 leaderboard 说明最稳；像 `DeepResearch Bench`、`BrowseComp`、`WebWalkerQA`、`WideSearch` 都能直接从官方来源提取任务规模、测量目标和已知局限，减少二手解读漂移。
- 使用 `lark-cli docs +create --markdown \"$(cat /tmp/xxx.md)\" --as user` 可以稳定把较长的结构化 Markdown 一次性写入飞书；写完再用 `docs +fetch` 回读验证，比只看创建成功响应更可靠。
- 对已有飞书文档补充链接信息时，用 `docs +update --mode replace_range --selection-by-title` 直接替换单个章节最稳；既能保留正文主体，又能把“参考链接”平滑升级成“官方入口导航”。

失败/风险经验：
- 当前沙箱内访问 `open.feishu.cn` 会因 DNS / socket 权限失败；涉及飞书真实读写时，需要直接申请越权网络执行，不能把这类环境报错误判成飞书鉴权或 scope 配置问题。
- `lark-cli` 当前版本是 `1.0.3`，执行过程中会持续提示 `1.0.10` 可更新；后续若遇到文档语法或 shortcut 行为异常，先留意是否和 CLI 版本漂移有关。

### 2026-04-21 飞书 Base 简历附件导入补充

成功经验：
- 向 Base 附件字段上传本地简历时，需要先 `+record-upsert` 创建记录，再用 `+record-upload-attachment` 写入附件字段；不能把本地文件直接塞进记录 JSON。
- `+record-upload-attachment --file` 要求文件路径是当前工作目录下的相对路径；处理 Windows/WSL 路径时，先解压到 `/tmp`，再把 `workdir` 切到文件所在目录并传 `./文件名.pdf` 最稳。
- 同一批 zip 里出现同名同 hash 的简历时，先用 `sha256sum` 去重再建档，可避免重复上传同一份附件；同名但旧流程已结束、岗位来源不同的记录可按本次导入新建。

失败/风险经验：
- `lark-cli --jq` 构造对象时中文 key 可能触发解析错误；核验记录时用英文输出 key，并通过 `.data.record["姓名"]` 这种方括号方式读取中文字段更稳。
- 当前 `lark-cli` 仍提示版本从 `1.0.3` 升级到 `1.0.15`；如果后续附件上传或 shortcut 返回结构异常，优先考虑 CLI 版本漂移。
