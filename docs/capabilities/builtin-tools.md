...
| `apply_patch` | 使用结构化 patch 格式修改一个或多个文件 | MEDIUM | builtin.py |
| `edit_file` | 对文件做精确单次文本替换 | HIGH | builtin.py |
...
**工作目录**：
- 未指定 `working_dir` 时默认使用 Agent 工作目录
...
通过 HTTP GET 获取 URL 内容，支持按 `Content-Type` 提取文本、Markdown 或下载非文本文件。
...
|------|------|------|------|
| `url` | string | 是 | 目标 URL |
| `format` | string | 否 | 输出格式，支持 `markdown` / `text`，默认 `markdown` |
...
```json
{
  "download_path": "/path/to/session/report.pdf",
  "download_filename": "report.pdf",
  "summary": "已下载非文本内容: application/pdf ..."
}
```

**保护机制**：
- URL 必须以 `http://` 或 `https://` 开头
- 内存保护截断：文本响应超过 `tools.fetch_url.max_response_mb`（默认 10 MB）时自动截断
- 返回结果仍由 ToolRuntime 层做 token 级截断
- 支持重定向跟随（`follow_redirects=True`）
- 返回值中 `url` 为最终重定向后的地址

**Content-Type 处理规则**：
- `application/json` / `*+json`：格式化 JSON 文本后返回
- `text/html` / `application/xhtml+xml`：提取正文；`format=markdown` 保留标题、链接、列表等基础结构
- `text/plain` / `text/markdown`：直接返回文本内容
- 其他非文本类型：下载到 session 附件目录，并返回摘要与本地路径

## read_file

读取文本文件内容，支持按行范围读取；读取图片文件时返回 base64 多模态内容块。
...
使用结构化 patch 格式对一个或多个文件执行增删改操作，适合多文件或多 hunk 修改。该工具当前对齐 `openclaw` 的 `apply_patch` 方言，只接受 `*** Begin Patch ... *** End Patch` 这一套格式。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `input` | string | 是 | 完整 patch 内容，必须使用 `*** Begin Patch/End Patch` 格式 |
...
**格式约束**：
- 合法 hunk 头只有 `*** Add File:`、`*** Delete File:`、`*** Update File:`
- `*** Update File:` 内可选 `*** Move to:` 做重命名
- `@@` 或 `@@ <context>` 用于 update chunk；首个 chunk 可省略上下文，后续 chunk 需要显式 `@@`
- `*** End of File` 用于 EOF 插入场景
- 支持 `<<EOF ... EOF` 包裹的 heredoc patch 边界

**注意事项**：
- 不要传 unified diff 头，例如 `File:`、`---`、`+++`、`@@ -1,3 +1,3 @@`；这类输入会直接按错误格式报错
- 相对路径优先基于 Agent `_agent_workdir` 解析，绝对路径按原路径处理
- 若运行时注入了 `_path_policy`，会在写入前做路径授权检查

## edit_file

对文本文件执行精确替换，要求 `oldText` 在目标文件中恰好命中一次。该工具适合“小范围、可验证”的修改，不适合整文件重写。
...
| `oldText` | string | 是 | 被替换的旧文本 |
| `newText` | string | 是 | 替换后的新文本 |
| `old_text` | string | 否 | `oldText` 的 snake_case 别名 |
| `new_text` | string | 否 | `newText` 的 snake_case 别名 |
...
{
  "success": true,
  "message": "Successfully replaced text in app.py.",
...
**失败语义**：
- `oldText` 未命中：拒绝编辑，避免误写
- `oldText` 命中多次：拒绝编辑，避免改错位置
- 目标不存在或不是普通文件：直接失败
- 相对路径优先基于 Agent `_agent_workdir` 解析

**兼容性说明**：
- 实现对齐当前仓库约定，工具名固定为 `edit_file`
- 当文件已写入成功但 diff 生成阶段异常时，会重新检查磁盘内容并尽量恢复为成功结果，避免假失败
