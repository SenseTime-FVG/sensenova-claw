...
**注意：需要后端改动。** 当前 `agent_worker._handle_user_input()` 只在首轮从 `load_workspace_files()` 加载 workspace 文件作为 `context_files`，不读取 event payload 中前端传来的 `context_files` 路径。需要修改 `agent_worker.py`：
1. 从 `event.payload.get("context_files", [])` 读取前端传来的文件路径列表
2. 对每个路径读取文件内容，转换为 `ContextFile(name, content)` 对象
3. 与现有的 workspace `context_files`（首轮加载）合并后传给 `context_builder.build_messages()`
4. 路径安全检查：复用 `PathPolicy` 确保只读取允许的路径
...
### 8.2 Agent Worker 消费 context_files
...
async def _handle_user_input(self, event: EventEnvelope) -> None:
    ...
    if user_file_paths:
        user_files = await self._load_user_context_files(user_file_paths)
...
文件列表 API 和 context_files 读取共用安全策略：
- `workspace/` 目录始终允许读取
- 用户指定的目录路径需通过前端传入，后端通过 `PathPolicy.check_read()` 验证
- 当 `PathPolicy` 返回 `NEED_GRANT` 时，API 返回 403，前端提示用户无权限
- 对所有路径做 `os.path.realpath()` 防止 symlink/`..\` 遍历
...
- DashboardNav 导航项
- RightContext 组件（保持现有 UI 设计，但数据源从 props 改为从 `ChatSessionProvider` context 读取）
- WebSocket 协议和后端事件格式
- Session API（`GET /api/sessions`、WebSocket create/load）
- 后端 AgentRuntime / LLMRuntime / ToolRuntime（除 agent_worker context_files 改动外）
