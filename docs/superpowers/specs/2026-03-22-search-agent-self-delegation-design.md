...
### 3. search-agent System Prompt 增强
...
- **硬兜底**：`max_send_depth: 2`，子 session (depth=1) 即使尝试再派发，孙 session (depth=2) 到达上限会被 `send_message` 拒绝
...
| # | 文件 | 类型 | 改动内容 |
|---|---|---|---|
| 1 | `agentos/capabilities/tools/send_message_tool.py` | 代码 | 循环检测条件加 `and target_id != current_agent_id` |
| 2 | `.agentos/agents/search-agent/config.yml` | 配置 | `max_send_depth: 1` → `2` |
| 3 | `.agentos/agents/search-agent/SYSTEM_PROMPT.md` | 配置 | 追加任务拆分与并行执行指引 |

## 测试策略

- **单元测试**：验证循环检测放宽后，自我委派通过、真正循环（A→B→A）仍被拦截
