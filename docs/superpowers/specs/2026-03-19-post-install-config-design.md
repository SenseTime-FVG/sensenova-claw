...
用户安装 AgentOS 后，如果未配置任何 LLM API，系统应在所有入口（终端、Web、CLI）提示用户完成配置。配置完成后引导用户使用 SystemAdmin Agent 进行更多系统管理操作。
...
**文件**：`agentos/app/main.py`（`run` 命令入口）
...
`agentos cli` 连接成功后，检测到未配置 LLM API。
...
请选择 LLM 提供商:
  1. OpenAI 兼容 (OpenAI/通义千问/智谱GLM/MiniMax/DeepSeek/零一万物等)
  2. Anthropic (Claude)
  3. Google Gemini
  4. 跳过配置
> 1
...
请输入 API Key: sk-xxxx
请输入 Base URL (回车使用默认值 https://api.openai.com/v1):
...
- **文件**：`agentos/app/cli/app.py` 中新增 `_run_llm_setup()` 方法
- 直接读写本地 config.yml 文件
- 各 provider 预设默认 base_url：
...
风格仿照 `/login` 页面（居中白色卡片，AgentOS 品牌标识）。

**步骤流程**：
1. 选择 Provider（按钮式选择，同 CLI 选项）
2. 输入 API Key + Base URL（Base URL 根据 provider 预填默认值）
3. 选择默认模型（下拉框，根据 provider 过滤）
4. 底部：「完成配置」按钮 + 「跳过」链接
...
**文件**：
- 新增 `agentos/app/web/app/setup/page.tsx`
- 修改 `agentos/app/web/components/ProtectedRoute.tsx`
...
`agentos/capabilities/skills/builtin/system-admin-skill/SKILL.md`
...
- 配置目录路径：读取 config.yml 中 `system.agentos_home` 字段，或检查环境变量 `AGENTOS_HOME`，默认为 `~/.agentos/`
- 通过 `bash_command` 执行 `echo $AGENTOS_HOME` 确定实际配置目录
...
- **查看 Agent 列表**：`bash_command` 执行 `ls {agentos_home}/agents/`
- **创建 Agent**：在 `{agentos_home}/agents/{id}/` 目录下创建 `config.json`、`AGENTS.md`、`USER.md`
...
- **启用/禁用工具**：修改 `{agentos_home}/.agent_preferences.json`

#### 5. Skill 管理

- **查看已安装 Skill**：`bash_command` 执行 `ls {agentos_home}/skills/`
- **安装 Skill**：`bash_command` 将 skill 目录复制到 `{agentos_home}/skills/`
- **卸载 Skill**：`bash_command` 删除对应 skill 目录
- **启用/禁用 Skill**：读写 `{agentos_home}/skills_state.json`
...
- **查看数据库大小**：`bash_command` 检查 `{agentos_home}/data/` 目录
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `agentos/interfaces/http/config_api.py` | 修改 | 新增 `GET /api/config/llm-status` |
| `agentos/app/main.py` | 修改 | `run` 命令启动后检测并打印提示 |
| `agentos/app/cli/app.py` | 修改 | 新增 `_run_llm_setup()` 交互式引导 |
| `agentos/app/web/app/setup/page.tsx` | 新增 | LLM 配置引导页 |
| `agentos/app/web/components/ProtectedRoute.tsx` | 修改 | 添加 LLM 状态检测和重定向 |
| `agentos/capabilities/agents/registry.py` | 修改 | 确保 system-admin Agent 始终注册 |
| `agentos/capabilities/skills/builtin/system-admin-skill/SKILL.md` | 新增 | 运维 Skill |
| `agentos/app/gateway/main.py` | 修改 | 启动时注册 system-admin Agent |
