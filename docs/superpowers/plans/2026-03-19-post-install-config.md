...
```python
# agentos/platform/config/llm_presets.py
...
**Files:**
- Create: `agentos/platform/config/llm_presets.py`
- Test: `tests/unit/test_llm_presets.py`

- [ ] **Step 1: 创建预设模块**

创建 `agentos/platform/config/llm_presets.py`，包含上述 `LLM_PROVIDER_CATEGORIES` 数据和辅助函数：
...
```bash
git add agentos/platform/config/llm_presets.py tests/unit/test_llm_presets.py
...
```bash
git add agentos/interfaces/http/config_api.py tests/unit/test_llm_status_api.py
...
**Files:**
- Modify: `agentos/app/main.py:122-134`
...
from agentos.platform.config.config import Config, PROJECT_ROOT
...
```bash
git add agentos/app/main.py
git commit -m "feat: agentos run 启动时检测 LLM 配置并提示"
```
...
**Files:**
- Create: `agentos/app/cli/llm_setup.py`
- Modify: `agentos/app/cli/app.py:360-376` (在 `_run_interactive_mode` 中调用)
...
```python
    async def _check_and_setup_llm(self) -> None:
        ...
        from agentos.app.cli.llm_setup import run_llm_setup
...
```bash
git add agentos/app/cli/llm_setup.py agentos/app/cli/app.py tests/unit/test_cli_llm_setup.py
...
**Files:**
- Modify: `agentos/capabilities/agents/registry.py:82-106`
- Create: `agentos/capabilities/skills/builtin/system-admin-skill/SKILL.md`
...
你是 AgentOS 的系统管理员。通过 `read_file`、`write_file`、`bash_command` 三个工具完成所有管理操作。

## 环境感知

在执行任何操作前，必须先确定配置目录：

1. 执行 `bash_command`: `echo $AGENTOS_HOME` 获取环境变量
2. 如果为空，读取 config.yml 中 `system.agentos_home` 字段
3. 如果仍为空，默认使用 `~/.agentos/`
4. 记住这个路径作为 `{AGENTOS_HOME}`，后续所有操作使用它
...
### 查看 Agent 列表
```bash
ls {AGENTOS_HOME}/agents/
```

### 创建 Agent
1. 创建目录：`mkdir -p {AGENTOS_HOME}/agents/{agent_id}`
2. 创建 `config.json`：
```json
{
  "id": "agent_id",
...
### 修改 Agent
读取 → 修改 → 写回 `{AGENTOS_HOME}/agents/{agent_id}/config.json`

### 删除 Agent
```bash
rm -rf {AGENTOS_HOME}/agents/{agent_id}
rm -rf {AGENTOS_HOME}/workdir/{agent_id}
```
...
### 启用/禁用工具
编辑 `{AGENTOS_HOME}/.agent_preferences.json`：
```json
{
  "tools": {
    "bash_command": true,
...
```bash
# 内置 Skill
ls agentos/capabilities/skills/builtin/
# 用户安装的 Skill
ls {AGENTOS_HOME}/skills/
```

### 安装 Skill（从本地路径）
```bash
cp -r /path/to/skill_dir {AGENTOS_HOME}/skills/{skill_name}
```
确保目标目录中包含 `SKILL.md` 文件。

### 卸载 Skill
```bash
rm -rf {AGENTOS_HOME}/skills/{skill_name}
```

### 启用/禁用 Skill
编辑 `{AGENTOS_HOME}/skills_state.json`：
```json
{
  "skill_name": true,
  "another_skill": false
...
```bash
ls -la {AGENTOS_HOME}/
ls -la {AGENTOS_HOME}/agents/
ls -la {AGENTOS_HOME}/skills/
```

### 查看数据库大小
```bash
ls -lh {AGENTOS_HOME}/data/
```
...
```bash
git add agentos/capabilities/skills/builtin/system-admin-skill/SKILL.md
...
**Files:**
- Create: `agentos/app/web/app/setup/page.tsx`
- Modify: `agentos/app/web/components/ProtectedRoute.tsx`

- [ ] **Step 1: 创建 /setup 页面**

创建 `agentos/app/web/app/setup/page.tsx`，风格仿照 `/login` 页面：
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md w-full space-y-6 p-8 bg-white rounded-lg shadow-md">
        <div>
          <h2 className="text-center text-3xl font-extrabold text-gray-900">AgentOS</h2>
...
```bash
git add agentos/app/web/app/setup/page.tsx agentos/app/web/components/ProtectedRoute.tsx
