...
| 文件 | 操作 | 职责 |
|------|------|------|
| `agentos/interfaces/ws/gateway.py` | 大改 | 新增统一业务 API，接管会话管理，依赖 repo/agent_registry |
| `agentos/adapters/channels/base.py` | 小改 | Channel 基类增加 gateway 属性 |
| `agentos/adapters/channels/websocket_channel.py` | 大改 | 新增 handle_connection()，内含认证+消息循环 |
| `agentos/app/gateway/main.py` | 大改 | 删除 WS 消息处理，简化 HTTP 端点 |
| `agentos/adapters/channels/feishu/channel.py` | 小改 | 适配 Gateway 新 API |
...
**Files:**
- Modify: `agentos/interfaces/ws/gateway.py`
...
Run: `python3 -c "from agentos.interfaces.ws.gateway import Gateway; print('OK')"`

- [ ] **Step 6: Commit**

```bash
git add agentos/interfaces/ws/gateway.py
...
**Files:**
- Modify: `agentos/adapters/channels/base.py`
...
```bash
git add agentos/adapters/channels/base.py agentos/interfaces/ws/gateway.py
...
**Files:**
- Modify: `agentos/adapters/channels/websocket_channel.py`
...
Run: `python3 -c "from agentos.adapters.channels.websocket_channel import WebSocketChannel; print('OK')"`

- [ ] **Step 6: Commit**

```bash
git add agentos/adapters/channels/websocket_channel.py agentos/adapters/channels/base.py
...
从 main.py 移除：
- `from agentos.kernel.events.types import USER_INPUT, USER_TURN_CANCEL_REQUESTED, TOOL_CONFIRMATION_RESPONSE`
- `from agentos.kernel.events.envelope import EventEnvelope`（如果其他地方不用）
- `from agentos.platform.security.middleware import verify_websocket`（已移入 Channel）
...
Run: `python3 -c "from agentos.app.gateway.main import app; print('OK')"`
...
**Files:**
- Modify: `agentos/adapters/channels/feishu/channel.py`
...
```bash
git add agentos/adapters/channels/feishu/channel.py
