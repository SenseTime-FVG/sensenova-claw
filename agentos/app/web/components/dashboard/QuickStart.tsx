...
function QuickCard({ item }: { item: (typeof quickItems)[number] }) {
  ...
  return (
    <Link href={item.path} className="group">
      <div className="relative overflow-hidden rounded-[22px] border border-white/80 bg-white/72 p-5 text-left shadow-[0_10px_26px_rgba(15,23,42,0.05)] backdrop-blur-xl transition duration-200 hover:-translate-y-0.5">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-[16px] border border-white/85 bg-white/80 text-lg shadow-sm">
              {item.icon}
            </div>
            <div>
