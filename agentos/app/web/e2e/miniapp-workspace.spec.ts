...
function mockAuthAndWebSocket() {
  document.cookie = 'agentos_token=e2e-agentos-token; path=/';
...
test('mini-app 工作区应在构建期转发 builder 消息并按 local/server/agent 分流页面动作', async ({ page }) => {
  ...
  await page.route('**/api/custom-pages/research-hub', async (route) => {
    ...
    const runningPayload = {
      ...
      bridge_script_path: 'miniapp-research-hub-agent/miniapps/research-hub/app/agentos-bridge.js',
  ...
  await page.getByLabel('Token').fill('e2e-agentos-token');
