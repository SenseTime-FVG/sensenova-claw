...
class TokenAuthService:
    ...
    def _write_token_file(self, home: Path) -> None:
        """将 token 写入 {agentos_home}/token，权限 600"""
        try:
...
def read_token_file(agentos_home: str | Path | None = None) -> str | None:
    """从 {agentos_home}/token 读取 token（供 CLI 等客户端使用）"""
    if not agentos_home:
        agentos_home = Path.home() / ".agentos"
    token_file = Path(agentos_home) / TOKEN_FILENAME
