...
class MiniAppService:
    ...
    def __init__(
        self,
        *,
        agentos_home: str | Path,
        config: Any,
        agent_registry: Any,
        gateway: Any | None = None,
    ) -> None:
        self.agentos_home = Path(agentos_home).expanduser().resolve()
        self.config = config
    ...
    def storage_path(self) -> Path:
        return self.agentos_home / "custom_pages.json"

    @property
    def runs_dir(self) -> Path:
        return self.agentos_home / "custom_pages_runs"
    ...
    async def create_page(self, payload: dict[str, Any]) -> dict[str, Any]:
        async with self._lock:
            ...
            page: dict[str, Any] = {
                ...
                "bridge_script_path": f"{self._workspace_rel_dir(effective_agent_id, slug)}/app/agentos-bridge.js",
    ...
    def _workspace_abs_dir(self, page: dict[str, Any]) -> Path:
        return self.agentos_home / "workdir" / str(page["workspace_root"])

    def _app_abs_dir(self, page: dict[str, Any]) -> Path:
        return self.agentos_home / "workdir" / str(page["app_dir"])
    ...
    async def _ensure_agent(self, page: dict[str, Any], *, refresh_existing: bool) -> None:
        ...
        await ensure_agent_workspace(str(self.agentos_home), agent_id)
        ...
        prompt_path = self.agentos_home / "agents" / agent_id / "SYSTEM_PROMPT.md"
    ...
    def _build_agent_system_prompt(self, page: dict[str, Any], base_prompt: str) -> str:
        ...
        return (
            ...
            "- 如果页面里要把交互发送回宿主页面，请使用 window.AgentOSMiniApp.emit(action, payload)"
        ).strip()
    ...
    def _build_acp_prompt(self, page: dict[str, Any], prompt: str) -> str:
        return (
            ...
            "2. 若需要把页面交互发送给宿主系统，请使用 window.AgentOSMiniApp.emit(action, payload)。\n"
    ...
    def _copy_source_project(self, page: dict[str, Any]) -> tuple[Path | None, list[str]]:
        ...
        for child in app_dir.iterdir():
            if child.name.startswith("."):
                continue
            if child.name in {"index.html", "app.js", "styles.css", "agentos-bridge.js"}:
    ...
    def _inject_bridge_script(self, entry_path: Path) -> None:
        try:
            ...
            return
        if "agentos-bridge.js" in content:
            return
        if "</body>" in content:
            content = content.replace(
                "</body>",
                '  <script src="./agentos-bridge.js"></script>\n</body>',
            )
        else:
            content += '\n<script src="./agentos-bridge.js"></script>\n'
    ...
    def _write_bridge_script(self, page: dict[str, Any]) -> None:
        bridge_path = self._app_abs_dir(page) / "agentos-bridge.js"
        bridge_path.write_text(_bridge_js(page["slug"]), encoding="utf-8")

    def _to_workdir_rel(self, path: Path) -> str:
        return str(path.resolve().relative_to((self.agentos_home / "workdir").resolve())).replace("\\", "/")
...
def _build_attributions_markdown(source: Path, licenses: list[str]) -> str:
    ...
    return (
        "# 项目复用归因\n\n"
        f"- 复用来源路径: `{source}`\n"
        "- 该 mini-app 由 AgentOS 在保留授权文件的前提下复制到当前工作区。\n"
        "- 请在继续修改时保留原授权与归因信息。\n\n"
        "## 已保留的授权文件\n"
        f"{license_lines}\n"
    )
  ...
  <body>
    <main class="placeholder-shell">
      <div class="placeholder-card">
        <span class="eyebrow">AgentOS Mini-App</span>
        <h1>{name}</h1>
...
function notifyAgent(action, payload) {{
  ...
  if (window.AgentOSMiniApp) {{
    window.AgentOSMiniApp.emit(action, payload);
  }}
...
if (window.AgentOSMiniApp && window.AgentOSMiniApp.configureActionRouting) {{
  window.AgentOSMiniApp.configureActionRouting(actionRouting);
}}
  ...
  <body>
    <main class="miniapp-shell">
      <section class="card hero-main">
        ...
        <div class="hero-actions">
          <button class="btn btn-primary" onclick="window.AgentOSMiniApp && window.AgentOSMiniApp.emit('request_project_adaptation', {{ source: '{source}' }})">让 Agent 继续接入这个项目</button>
        </div>
      </section>
    </main>
    <script src="./agentos-bridge.js"></script>
