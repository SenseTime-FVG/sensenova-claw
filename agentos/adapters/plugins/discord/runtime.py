...
class DiscordRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self._agentos_status: dict[str, str] = {"status": "idle"}
    ...
    async def start(self) -> None:
        if not self._config.bot_token:
            self._agentos_status = {"status": "failed", "error": "missing bot token"}
            raise RuntimeError("Discord bot token is required")

        try:
            discord = importlib.import_module("discord")
        except ImportError as exc:
            self._agentos_status = {"status": "failed", "error": "discord.py not installed"}
        ...
        class _AgentOSDiscordClient(discord.Client):
            async def on_ready(self) -> None:  # type: ignore[override]
                runtime._agentos_status = {"status": "ready"}
        ...
        self._client = _AgentOSDiscordClient(intents=intents)
        self._agentos_status = {"status": "connecting"}
        try:
            ...
            self._agentos_status = {"status": "failed", "error": format_discord_runtime_error(exc)}
            raise
    ...
    async def stop(self) -> None:
        ...
        self._agentos_status = {"status": "stopped"}
