...
- 确认 `deck_dir/images/` 目录存在，不存在则创建。不要假设 `images/` 已存在。
...
- 一页需要多张独立真实图片时，必须拆成多个槽位；`asset-plan.json` 中的 `real-photo` 槽位数量不能少于该页实际需求。三张人物卡不能共享一个槽位，双图展示页不能只落一个 `selected_image`。
