...
- `storyboard.json.pages[n].payload_budget` 是否被兑现
...
- 以上任一硬性项失败，必须判定为阻塞，不得交付。
- `构图` 与 `风格一致性` 属于弱筛查；没有明显大问题时可以通过，但要在 review 中留下告警或备注。
- 必须逐页对照 `payload_budget.claim_count`、`payload_budget.evidence_count`、`payload_budget.structure_block_count`、`payload_budget.require_comparison_or_summary` 审查最终页面。
- 如果页面承载不足、结构块不足，或预算要求了对比 / 摘要但页面没有兑现，必须明确记为页面级问题。
...
- 这类问题应转到 `ppt-page-assets` 或回到 `ppt-asset-plan` 继续补齐。
