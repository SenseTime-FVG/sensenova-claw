...
| source_plan | primary: serper_search + fetch_url; supplement: union-search-skill / 补充搜索 skill（仅主链不足时） | — |
...
搜索源优先级：`serper_search`（主链） > 内置搜索工具（brave/tavily 等） > `union-search-skill` / 补充搜索 skill（仅主链不足时）
...
**需要用户确认的情况**：研究范围需明显扩大、需启用 union-search-skill 或补充搜索 skill（如 search-code、search-social-cn 等）、发现前提假设可能有误。
...
## 与 union-search-skill / 补充搜索 Skill 的衔接
...
- 简单问题强行走复杂流程
- PLAN 未确认就大规模搜索
- union-search-skill 或补充搜索 skill 当默认主链
- 编造来源或伪造引用
- "搜索到"等同于"已验证"
- 省略关键结论的来源链接
