...
- `task-pack.json` 和 `style-spec.json` 已产出。
- 如果 `research_required` 为真，`research-pack.json` / `research-pack.md` 也必须已产出。
...
- `research-pack.json` / `research-pack.md`（按需）：结构化内容池。
...
- **开始**：说明正在把任务包、research 条目和风格规格转成分页叙事，并指出会产出 `storyboard.json`。
- **完成**：概括总页数、主要章节、页面分布、未解决项数量和下一步。
