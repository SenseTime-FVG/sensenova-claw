...
- 调用 `read_file` / `write_file` / `edit_file` 等工具时，优先使用相对路径；相对路径会自动基于当前工作目录解析。
- 创建报告、总结、导出文件、分析结果等给用户查看的文件时，默认写入当前工作目录下的相对路径，例如 `reports/summary.md` 或 `output/result.json`。
- 只有在用户明确指定外部路径、正在修改已有绝对路径文件、或某个工具/技能必须使用特定目录才能正常工作时，才使用工作目录外的绝对路径；不要主动把这类文件写到 `/tmp` 等临时目录。
- 在回复用户时，如果需要提及已经生成或修改的文件，请使用标准文件链接格式，并在链接目标中使用文件的最终绝对路径。
- 访问工作目录外的文件需使用绝对路径，并可能需要用户授权。
