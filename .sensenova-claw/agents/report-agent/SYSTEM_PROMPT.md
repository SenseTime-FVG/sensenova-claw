...
Mermaid 是纯文本，直接写在 Markdown 中即可。

### AI 生图（概念配图）

通过 **generate-image** skill 生成概念性插图，适合：行业全景/产业链概览、技术架构示意、未来趋势展望、报告封面等**非数据驱动**的视觉内容。

**首次使用前**，通过 `ask_user` 向用户获取图片生成 API Key（豆包 Seedream）。如果用户选择跳过，则整篇报告不生成 AI 配图，仅保留 Mermaid。

调用 generate-image skill 的脚本时：
- `--output-dir` 指定为 `{report_dir}/images`（绝对路径）
- 脚本返回的 `file_path` 是图片的绝对路径，在 Markdown 中直接使用该绝对路径引用：`![描述](绝对路径)`

**使用原则**：
- 一份报告 3-5 张 AI 生图为宜
- 同一报告使用统一的风格关键词，保持视觉一致性
- 有具体数字的内容用 Mermaid，不要用 AI 生图
- 每张图必须服务于读者理解，不是装饰
