...
- **research-agent**: 按指定维度搜集证据,输出结构化的 evidence.json
...
- **Controller 只调度,不阅读大文件**——evidence.json / sections/*.md / stitched.md 全部通过文件路径传递,agent 自己 Read
- **outline.json 是报告阶段的唯一契约**——planner 产出,writer / stitcher / render 直接消费
- **sections/s{N}.evidence_subset.json 是 writer 的工作边界**——硬隔离,writer 看不到原始 evidence.json
- **每条 schema 校验必须过 validator**——controller 自己**不**判断字段是否合规,由 validator 守门
...
- 开始研究时，基于你的工作目录构造 `report_dir` 的绝对路径：`{你的工作目录}/reports/YYYY-MM-DD-{topic}/`，如果已有同名目录，请重新命名
...
├── sub_reports/
│   ├── d1.evidence.json     # research 阶段产出(真相来源)
│   ├── d2.evidence.json
│   └── ...
├── report.md                # 终稿（引用预处理后为 [N] 编号 + 参考文献列表）
└── citations.json           # 引用元数据（引用预处理后生成）
```

开始前在工作目录下创建报告目录:

```bash
mkdir -p {report_dir}/sub_reports
```
...
**输出路径**：完成后请使用 write_file 写入 {report_dir}/sub_reports/{dimension_id}.evidence.json
```

#### 3b. 审查

每份 evidence.json 保存后，发送给 review-agent，最多审查3次。
...
**evidence 文件路径**：{report_dir}/sub_reports/{dimension_id}.evidence.json
请使用 read_file 读取后审查。
```
...
**输出路径**：修订完成后使用 write_file 覆写 {report_dir}/sub_reports/{dimension_id}.evidence.json
```
