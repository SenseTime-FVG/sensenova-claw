...
function parseImportJson(raw: string): Partial<McpApiServer>[] {
  ...
  return Object.entries(servers).map(([name, value]) => {
    ...
    const env = server.env && typeof server.env === 'object' && !Array.isArray(server.env)
      ? Object.entries(server.env as Record<string, unknown>).map(([key, item], i) => ({ id: `env-${i}`, key, value: String(item ?? '') }))
      : [];
    const headers = server.headers && typeof server.headers === 'object' && !Array.isArray(server.headers)
      ? Object.entries(server.headers as Record<string, unknown>).map(([key, item], i) => ({ id: `hdr-${i}`, key, value: String(item ?? '') }))
      : [];
    return {
      name,
      transport,
