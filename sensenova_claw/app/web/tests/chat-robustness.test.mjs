...
test('聊天页面应提供自动批准所有工具开关并由 InteractionContext 自动放行', () => {
  const dialogSource = readSource('components/chat/QuestionDialog.tsx');
  const interactionSource = readSource('contexts/ws/InteractionContext.tsx');
  const chatPageSource = readSource('app/chat/page.tsx');
  const chatPanelSource = readSource('components/chat/ChatPanel.tsx');
  const chatInputSource = readSource('components/chat/ChatInput.tsx');

  assert.doesNotMatch(dialogSource, /tool-confirm-auto-approve-all/);
  assert.doesNotMatch(chatPageSource, /data-testid="tool-auto-approve-toggle"/);
  assert.doesNotMatch(chatPanelSource, /data-testid="tool-auto-approve-toggle"/);
  assert.match(chatInputSource, /data-testid="tool-auto-approve-toggle"/);
  assert.match(chatInputSource, /自动批准工具/);

  assert.match(interactionSource, /TOOL_AUTO_APPROVE_STORAGE_KEY/);
  assert.match(interactionSource, /isToolAutoApproveEnabledRef\.current/);
