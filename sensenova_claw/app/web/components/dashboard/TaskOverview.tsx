...
function SummaryCard({ item }: { item: SummaryItem }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[26px] border border-[var(--glass-border-heavy)] bg-[var(--glass-bg)] p-5 shadow-[0_14px_36px_rgba(15,23,42,0.05)] dark:shadow-[0_14px_36px_rgba(0,0,0,0.2)] backdrop-blur-xl">
      ...
      <div className="relative z-10 flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-3 text-4xl font-semibold tracking-tight text-[var(--glass-text)]">
            {item.value}
          </div>
          <div className="mt-1 text-xs text-muted-foreground">{item.desc}</div>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-[18px] border border-[var(--glass-border-heavy)] bg-[var(--glass-bg)] text-lg shadow-sm backdrop-blur-xl">
          {item.icon}
        </div>
      </div>
    </div>
