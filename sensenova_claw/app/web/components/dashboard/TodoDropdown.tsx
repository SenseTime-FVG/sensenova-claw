...
export function TodoDropdown() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-2xl border border-white/80 bg-white/95 p-3 shadow-[0_20px_60px_rgba(15,23,42,0.12)] backdrop-blur-2xl z-[100]">
          <div className="mb-2 flex items-center justify-between px-1">
            <span className="text-sm font-semibold text-neutral-800">今日待办</span>
            <span className="rounded-full bg-violet-500/10 px-2 py-0.5 text-[11px] font-medium text-violet-700">
              {pendingCount} 项待完成
            </span>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex h-20 items-center justify-center">
              <span className="text-xs text-neutral-400">暂无待办</span>
            </div>
          ) : (
              ...
              {items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-2.5 rounded-xl px-2 py-2 text-left transition-colors hover:bg-neutral-50"
                  >
                    {/* Checkbox */}
                    <div
