        ...
        <div className="min-w-0 flex-1">
          ...
          {!card.resolved && !card.pending && card.kind === 'user_question' && card.questionData && (
            <AskUserResponseForm
              className="mt-2"
              value={{
                question: card.questionData.question,
                options: card.questionData.options,
                multiSelect: card.questionData.multiSelect,
              }}
              testIdPrefix={`notification-ask-user-${card.interactionId || card.id}`}
              onSubmit={(answer) => onResolve(card.id, answer, answer)}
              onCancel={() => onResolve(card.id, '__cancelled__')}
            />
          )}
          ...
          {!card.resolved && !card.pending && card.kind !== 'user_question' && card.allowsInput && (
