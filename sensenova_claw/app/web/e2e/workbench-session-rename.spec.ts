...
test('工作台会话支持右键重命名', async ({ page }) => {
  const token = readCurrentToken();
  const patchCalls: Array<{ url: string; body: { title: string } }> = [];
  ...
  await page.route('**/api/agents', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify([{ id: 'default', name: '办公主助手', description: 'main' }]),
    });
  });
  ...
  await page.route('**/api/sessions**', async (route) => {
    ...
    if (request.method() === 'PATCH' && url.pathname === '/api/sessions/sess-rename-1') {
  ...
  await expect(page.getByText('原始会话标题')).toBeVisible();
  await page.getByTestId('workbench-session-item-sess-rename-1').click({ button: 'right' });
  await expect(page.getByTestId('workbench-session-context-menu')).toBeVisible();
  await page.getByTestId('workbench-session-context-menu-rename').click();
  await page.getByTestId('workbench-rename-input-sess-rename-1').fill('新的会话标题');
  await page.getByTestId('workbench-rename-input-sess-rename-1').press('Enter');

  await expect.poll(() => patchCalls.at(-1)?.body.title ?? '').toBe('新的会话标题');
