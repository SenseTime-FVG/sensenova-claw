...
test('消息页会话支持右键重命名', async ({ page }) => {
  ...
  await page.route('**/api/agents', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify([{ id: 'default', name: '办公主助手', description: 'main', status: 'active', model: 'mock' }]),
    });
  });
  ...
  await page.route('**/api/sessions**', async (route) => {
          ...
          sessions: [
            {
              ...
              meta: JSON.stringify({ title, agent_id: 'default' }),
              status: 'active',
              has_children: false,
            },
          ],
    ...
    if (request.method() === 'PATCH' && url.pathname === '/api/sessions/sess_chat_001') {
  ...
  await expect(page.getByTestId('chat-session-item-sess_chat_001')).toContainText('聊天旧标题');
  await page.getByTestId('chat-session-item-sess_chat_001').click({ button: 'right' });
  await expect(page.getByTestId('chat-session-context-menu')).toBeVisible();
  await page.getByTestId('chat-session-context-menu-rename').click();
  await page.getByTestId('chat-rename-input-sess_chat_001').fill('聊天新标题');
  await page.getByTestId('chat-rename-input-sess_chat_001').press('Enter');

  await expect.poll(() => patchCalls.at(-1) ?? '').toBe('聊天新标题');
  await expect(page.getByTestId('chat-session-item-sess_chat_001')).toContainText('聊天新标题');
});
