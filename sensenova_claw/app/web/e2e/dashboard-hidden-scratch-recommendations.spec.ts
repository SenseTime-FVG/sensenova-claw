...
test('hidden scratch recommendation 应继续使用原始 source session', async ({ page }) => {
            ...
            this.onmessage?.(new MessageEvent('message', {
              data: JSON.stringify({
                ...
                payload: {
                  ...
                  scratch_session_id: 'sess_scratch_001',
  ...
  await expect.poll(() => {
    const message = sentMessages.find((item) => item.type === 'load_session') as
      | { payload?: { session_id?: string } }
      | undefined;
    return message?.payload?.session_id;
  }).toBe('sess_source_001');

  await page.getByTestId('send-button').click();

  await expect.poll(() => {
    const message = sentMessages.find((item) => item.type === 'user_input') as
      | { session_id?: string }
      | undefined;
