...
def _write_config(
    *,
    ...
    if secret_store is not None:
        secret_ref = f"sensenova_claw/llm.providers.{storage_provider_key}.api_key"
...
def run_llm_setup_sync(config_path: Path) -> bool:
    ...
    console.print(
        Panel(
            ...
            "Sensenova-Claw 需要至少一个 LLM 提供商的 API Key 才能正常工作。\n"
            "以下引导将帮助您完成基础配置。",
            title="LLM 配置向导",
            border_style="yellow",
        )
    ...
    console.print("[dim]请重启 Sensenova-Claw 以使配置生效。[/dim]")
